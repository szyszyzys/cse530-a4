...
void read_input(FLOAT *vect, int grid_rows, int grid_cols, char *file)
{
	...
	if (!fp)
    {
        char err_msg[] = "file could not be opened for reading";
        printf(err_msg);
		fatal (err_msg);
    }
	...
	for (i=0; i < grid_rows * grid_cols; i++) {
		...
		if (feof(fp))
        {
            char err_msg[] = "not enough lines in file";
		    fatal (err_msg);
        }
        if ((sscanf(str, "%f", &val) != 1) )
        {
            char err_msg[] = "invalid file format";
		    fatal (err_msg);
        }
		vect[i] = val;
	}
...
int main(int argc, char **argv)
{
	...
	if(!temp || !power)
    {
        char err_msg[] = "unable to allocate memory";
		fatal(err_msg);
    }
